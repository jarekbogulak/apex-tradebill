import { makeTradeHistoryManager } from '../tradeHistory.usecases.js';
import {
...
describe('Trade history manager', () => {
  test('filters out calculations beyond the retention window', async () => {
    ...
    const historyManager = makeTradeHistoryManager({
      tradeCalculations: repository,
      now: () => new Date(),
    });

    const history = await historyManager.list(USER_ID, 10);
    expect(history.items).toHaveLength(1);
    expect(history.items[0].id).toBe(recent.id);
    expect(historyManager.isPersistent).toBe(true);
  });

  test('marks manager as non-persistent when configured', async () => {
    ...
    expect(historyManager.isPersistent).toBe(false);

    historyManager.setPersistent(true);
    expect(historyManager.isPersistent).toBe(true);
  });
});
