...
import type { TradePreviewService } from '../../services/trades/previewService.js';
import {
  createErrorResponse,
  errorResponseSchema,
  resolveUserId,
...
interface PostExecuteRouteOptions {
  previewService: TradePreviewService;
}
  ...
  app.post(
    ...
    async (request, reply) => {
      ...
      try {
        ...
        const result = await previewService.execute(userId, parseResult.data);
